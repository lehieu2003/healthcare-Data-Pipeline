...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-count", action="store_true", help="Skip pre-write count for large datasets.")
...
def main() -> None:
    ...
    try:
        ...
        row_count = None if args.skip_count else silver_df.count()
        write_silver_parquet(silver_df, args.output_path)
        row_count_text = "skipped" if row_count is None else str(row_count)
        print(f"Bronze to Silver complete year={args.year} rows={row_count_text} output={args.output_path}")
    finally:
        spark.stop()
